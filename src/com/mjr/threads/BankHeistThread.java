...
public class BankHeistThread extends Thread {
	...
	while (GameActive) {
	    ...
	    Utilities.sendMessage(type, channelName, messages[0]);
	    stage = 1;
	    try {
		Thread.sleep(10000);
	    } catch (InterruptedException e) {
	    ...
	    if (stage == 1) {
		Utilities.sendMessage(type, channelName, messages[1]);
		stage = 2;
		try {
		    Thread.sleep(60000);
		} catch (InterruptedException e) {
		...
		if (stage == 2) {
		    ...
		    Utilities.sendMessage(type, channelName, messages[randNum]);
		    if (randNum == 2) {
			stage = 3;
			try {
			    Thread.sleep(60000);
			...
			if (stage == 3) {
			    ...
			    if (randNum2 == 1)
				Utilities.sendMessage(type, channelName, messages[3]);
			    else
				Utilities.sendMessage(type, channelName, messages[5]);
			    path = 2;
			}
		    } else if (randNum == 4) {
			path = 1;
