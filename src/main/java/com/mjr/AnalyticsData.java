	...
	public static void sendData() {
		try {
			if (!MySQLConnection.executeQueryNoOutput("SELECT * FROM analytics").next()) {
				...
				MySQLConnection.executeUpdate("INSERT INTO analytics(name, value) VALUES (" + "\"" + "NumOfPointsRemoved" + "\"" + "," + "\"" + 0 + "\"" + ")");
			} else {
				ResultSet result = MySQLConnection.executeQuery("SELECT value FROM analytics WHERE name = " + "\"" + "NumOfCommandsUsed" + "\"");
				if (result.next()) {
					MySQLConnection.executeUpdate("UPDATE analytics SET value=" + (result.getInt(1) + AnalyticsData.getNumOfCommandsUsed()) + " WHERE name=" + "\"" + "NumOfCommandsUsed" + "\"");
				...
				if (result.next()) {
					MySQLConnection.executeUpdate("UPDATE analytics SET value=" + (result.getInt(1) + AnalyticsData.getNumOfMessagedModerated()) + " WHERE name=" + "\"" + "NumOfMessagedModerated" + "\"");
				...
				if (result.next()) {
					MySQLConnection.executeUpdate("UPDATE analytics SET value=" + (result.getInt(1) + AnalyticsData.getNumOfPointsGained()) + " WHERE name=" + "\"" + "NumOfPointsGained" + "\"");
				...
				if (result.next()) {
					MySQLConnection.executeUpdate("UPDATE analytics SET value=" + (result.getInt(1) + AnalyticsData.getNumOfPointsRemoved()) + " WHERE name=" + "\"" + "NumOfPointsRemoved" + "\"");
