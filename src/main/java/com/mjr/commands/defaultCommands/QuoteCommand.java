...
public class QuoteCommand extends Command {
	...
	public void onCommand(BotType type, Object bot, String channel, String sender, String login, String hostname, String message, String[] args) {
		if (Config.getSetting("Quotes", channel).equalsIgnoreCase("true")) {
			...
			} else if (args.length == 3 && args[1].equalsIgnoreCase("get") && Permissions.hasPermission(bot, type, channel, sender, PermissionLevel.Moderator.getName())) {
			...
			} else if (args.length > 1 && args[1].equalsIgnoreCase("add") && Permissions.hasPermission(bot, type, channel, sender, PermissionLevel.Moderator.getName())) {
				if (!message.contains("@")) {
					...
					ConsoleUtil.textToConsole(bot, type, channel, "A new Quote has been added by " + sender + ". The quote message is " + message, MessageType.ChatBot, null);
					EventLog.addEvent(channel, sender, "Added a new quote", EventType.Quote);
				}
			} else if (args.length == 2 && args[1].equalsIgnoreCase("help")) {
				Utilities.sendMessage(type, channel, "@" + sender + " Quote Help: You can use the following commands: !quote or !quote add <message> @<username> or !quote get <number>");
			}
		}
	}
