...
public class PointsSystemManager extends FileBase {
	...
	public static void migrateFile(BotType type, Object bot) {
		...
		for (Object user : properties.keySet()) {
			if (type == BotType.Twitch)
				MySQLConnection.executeUpdate("INSERT INTO points(name, twitch_channel_id, amount) VALUES (" + "\"" + ((String) user) + "\"" + "," + "\"" + MJRBotUtilities.getChannelIDFromBotType(type, bot) + "\"" + "," + "\""
						+ properties.getProperty((String) user) + "\"" + ")");
			else if (type == BotType.Mixer)
				MySQLConnection.executeUpdate("INSERT INTO points(name, mixer_channel, amount) VALUES (" + "\"" + ((String) user) + "\"" + "," + "\"" + MJRBotUtilities.getChannelNameFromBotType(type, bot) + "\"" + "," + "\""
						+ properties.getProperty((String) user) + "\"" + ")");
		}
	}
}
