				...
				if (twitchBot.mathsGame.isGameActive == true) {
					if (args.length == 2) {
						...
						if (twitchBot.mathsGame.Answer == index) {
							...
							ChatBotManager.sendMessage(type, bot, "@" + sender + " Well done, You have got the right answer! You have gained " + profit + " points!");
							EventLogManager.addEvent(type, bot, sender, "Won the Maths Game", EventType.Games);
							twitchBot.mathsGame.isGameActive = false;
						} else {
							ChatBotManager.sendMessage(type, bot, sender + " you have got the wrong answer try again!");
							EventLogManager.addEvent(type, bot, sender, "Got the wrong answer on the Maths Game", EventType.Games);
						}
					} else {
						ChatBotManager.sendMessage(type, bot, "@" + sender + " Invalid arguments! You need to enter !answer YOURANSWER (Example !answer 10)");
					}
				} else {
					ChatBotManager.sendMessage(type, bot, "@" + sender + " The maths game is currently not active!");
				}
