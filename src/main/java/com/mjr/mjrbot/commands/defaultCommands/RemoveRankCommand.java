		...
		if (ChannelConfigManager.getSetting("Ranks", type, bot).equalsIgnoreCase("true")) {
			if (args.length == 2) {
				...
				if (RankSystemManager.isOnList(user, type, bot)) {
					...
					ChatBotManager.sendMessage(type, bot, "@" + sender + " Removed " + user + " rank");
				} else {
					ChatBotManager.sendMessage(type, bot, "@" + sender + " Unable to remove " + user + " rank");
				}
			} else {
				ChatBotManager.sendMessage(type, bot, "@" + sender + " Invalid arguments! You need to enter !removerank USER");
			}
		}
	}
