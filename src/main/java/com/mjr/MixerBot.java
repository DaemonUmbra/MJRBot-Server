...
public class MixerBot extends MJR_MixerBot {
	...
	try {
		...
		ResultSet set = MySQLConnection
			.executeQueryNoOutput("SELECT * FROM tokens WHERE channel = '" + channel + "' AND platform = 'Mixer'");
	...
	if (type.equalsIgnoreCase("subscribed")) {
	    ...
	    ConsoleUtil.textToConsole(this, BotType.Mixer, this.channelName, user + " just subscribed to the channel!", MessageType.Bot,
		    null);
		...
		Utilities.sendMessage(BotType.Mixer, this.channelName,
			user + " just resubscribed to the channel for " + months + " months in a row!");
	    ConsoleUtil.textToConsole(this, BotType.Mixer, this.channelName,
		    user + " just resubscribed to the channel for " + months + " months in a row!", MessageType.Bot, null);
	    EventLog.addEvent(this.channelName, user, "Just resubscribed to the channel for " + months + " months in a row!",
		    EventType.Sub);
	    this.subscribers.add(user);
